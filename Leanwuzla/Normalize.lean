...
public def IO.printlnAndFlush {α} [ToString α] (a : α) : IO Unit := do
  IO.println a
  (← IO.getStdout).flush

namespace Lean.Meta.Tactic.BVDecide.Normalize
...
where
  ...
  runPipeline : PreProcessM Bool := do
    ...
    if cfg.structures || cfg.enums then
      let t1 ← IO.monoNanosNow
      let solved ← typeAnalysisPass.run
      let t2 ← IO.monoNanosNow
      IO.printlnAndFlush s!"[time] {typeAnalysisPass.name}: {t2 - t1}"
      if solved then return true

    let t1 ← IO.monoNanosNow
    let solved ← reductionPass.run
    let t2 ← IO.monoNanosNow
    IO.printlnAndFlush s!"[time] {reductionPass.name}: {t2 - t1}"
    if solved then return true
    ...
    if cfg.fixedInt then
      let t1 ← IO.monoNanosNow
      let solved ← intToBitVecPass.run
      let t2 ← IO.monoNanosNow
    ...
    if cfg.shortCircuit then
      let t1 ← IO.monoNanosNow
      let solved ← shortCircuitPass.run
      let t2 ← IO.monoNanosNow
